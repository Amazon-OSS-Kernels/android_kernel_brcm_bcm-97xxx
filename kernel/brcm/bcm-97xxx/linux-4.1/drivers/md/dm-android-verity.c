...
static int is_veritymode_disabled(void);
...
static int is_veritymode_disabled(void)
{
	...
	if (!value)
		value = veritymode;
	return (strncmp(value, enforcing, sizeof(enforcing) - 1) &&
		strncmp(value, eio, sizeof(eio) - 1) &&
		strncmp(value, logging, sizeof(logging) - 1));
}
...
static int android_verity_ctr(struct dm_target *ti, unsigned argc, char **argv)
{
	...
	*verity_table_args[VERITY_TABLE_ARGS + 2 + VERITY_TABLE_OPT_FEC_ARGS];
	...
	if (is_eng() || is_veritymode_disabled()) {
