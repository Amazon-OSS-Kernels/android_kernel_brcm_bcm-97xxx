...
static int bcm7038_wdt_set_wd_ctrl(struct watchdog_device *wdog, u32 val)
{
	struct bcm7038_watchdog *wdt = watchdog_get_drvdata(wdog);
	...
	writel(val, wdt->base + WDT_CTRL_REG);
	return 0;
}
...
static int bcm7038_wdt_probe(struct platform_device *pdev)
{
	...
	bcm7038_wdt_set_wd_ctrl(&wdt->wdd, WDT_EVENT_NMI);
	bcm7038_wdt_set_timeout(&wdt->wdd, bcm7038_wdt_get_timeleft(&wdt->wdd));
