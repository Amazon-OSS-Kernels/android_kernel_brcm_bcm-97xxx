...
{
	struct thermal_cooling_device *cdev[NR_CPUS];
	struct cpumask mask_val[NR_CPUS];
	int cpu, nr_cpu, cool_dev_idx;

	nr_cpu = num_possible_cpus();
	cool_dev_idx = 0;

	dev_info(&pdev->dev, "Number of CPU detected: %d\n", nr_cpu);
	if (NR_CPUS < (nr_cpu / 2)) {
		dev_err(&pdev->dev, "NR_CPUS < (nr_cpu / 2) !! \n");
			return PTR_ERR(NULL);
	}

	for (cpu = 0; cpu < nr_cpu; cpu += 2) {
		if (!cpufreq_frequency_get_table(cpu))
			return -EPROBE_DEFER;

		cpumask_set_cpu(cpu, &mask_val[cool_dev_idx]);
		cpumask_set_cpu(cpu + 1, &mask_val[cool_dev_idx]);

		cdev[cool_dev_idx] = cpufreq_cooling_register(&mask_val[cool_dev_idx]);

		if (IS_ERR(cdev[cool_dev_idx])) {
			dev_err(&pdev->dev, "Failed to register cooling device\n");
			return PTR_ERR(cdev[cool_dev_idx]);
		}
		dev_info(&pdev->dev, "Cooling device registered: %s\n",	cdev[cool_dev_idx]->type);
		cool_dev_idx++;
	}
