...
static int do_bsp_initiate_command(enum bsp_initiate_command cmd)
{
	...
	if ((ret & 0xff) != PM_INITIATE_SUCCESS)
		pr_err("error: pm_initiate returned %x\n", ret);
...
extern void sata_hdd_poweroff_ready(void);
		...
		unsigned long action, void *data)
{
	...
	pr_info("call sata_hdd_poweroff_ready from panic handler\n");
	sata_hdd_poweroff_ready();
#ifdef CONFIG_RAMDUMP
	in_panic = 1;
#endif
	return NOTIFY_DONE;
...
void amzn_preserve_ramoops_data( void )
{
	// if called, will signal BOLT to preserve
	// the ramoops data for use on next boot
	__raw_writel(BRCMSTB_PANIC_MAGIC,
		ctrl.aon_sram + AON_REG_PANIC);
