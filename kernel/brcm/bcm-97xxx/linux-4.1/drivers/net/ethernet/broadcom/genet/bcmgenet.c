...
static netdev_tx_t bcmgenet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	ring->prod_index &= DMA_P_INDEX_MASK;
        ...
        if (!(priv->started_flags & (1 << index))) {
                if (!bcmgenet_umac_readl(priv, UMAC_GTPKT))
                        netif_tx_stop_queue(txq);
                else
                        priv->started_flags |= (1 << index);
        }
...
static void bcmgenet_netif_stop(struct net_device *dev)
{
	...
	cancel_work_sync(&priv->bcmgenet_reset_work);
...
/* Reset bottom half */
static void bcmgenet_reset_task(struct work_struct *work)
{
       struct bcmgenet_priv *priv = container_of(
                       work, struct bcmgenet_priv, bcmgenet_reset_work);
       u32 reg;

       netif_err(priv, intr, priv->dev, "%s Resetting UniMAC module\n", __func__);
       ...
       /* issue soft reset with (rg)mii loopback to ensure a stable rxclk */
       bcmgenet_umac_writel(priv, reg | CMD_SW_RESET | CMD_LCL_LOOP_EN,
                            UMAC_CMD);
       udelay(2);
       bcmgenet_umac_writel(priv, reg, UMAC_CMD);
...
static void bcmgenet_timeout(struct net_device *dev)
{
	...
	schedule_work(&priv->bcmgenet_reset_work);
}
...
static int bcmgenet_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&priv->bcmgenet_reset_work, bcmgenet_reset_task);
