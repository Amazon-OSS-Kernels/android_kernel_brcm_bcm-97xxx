...
#define SATA_METRICS_STR_LEN 256
char *sata_metrics_prefix = "frankhdd";
...
static void ata_eh_link_report(struct ata_link *link)
{
	...
	int tag, nr_failed = 0;
#ifdef CONFIG_AMAZON_METRICS_LOG
	static int sata_command_error_count = 0;
#endif
...
#ifdef CONFIG_AMAZON_METRICS_LOG
			char buf[SATA_METRICS_STR_LEN];
			sata_command_error_count++;
			snprintf(buf, SATA_METRICS_STR_LEN,"%s:%s:ata_link_failed_command_error_count=%d;CT;1:NR",
				sata_metrics_prefix, product_id2? product_id2: "def",sata_command_error_count);
 ...
 retry:
	...
	if (reset) {
		...
		if (slave && reset == hardreset) {
			...
			ata_link_info(slave, "call local_error_recovery() when hard resetting link\n");
		   ...
		   struct ata_link **r_failed_link)
{
 ...
 retry:
	...
	ata_for_each_link(link, ap, EDGE) {
		...
		if (rc) {
			ata_link_err(link, "call local_error_recovery() before giving up\n");
			local_error_recovery();
