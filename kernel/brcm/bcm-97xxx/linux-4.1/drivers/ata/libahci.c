...
char *libahci_hdd_metrics_prefix = "frankhdd";
...
#ifdef CONFIG_AMAZON_METRICS_LOG
static ssize_t ahci_show_softreset_count(struct device *dev,
				      struct device_attribute *attr, char *buf);
#endif
...
#ifdef CONFIG_AMAZON_METRICS_LOG
static DEVICE_ATTR(ahci_softreset_count, S_IRUGO, ahci_show_softreset_count, NULL);
#endif

extern bool local_error_recovery(void);
...
#ifdef CONFIG_AMAZON_METRICS_LOG
static int softreset_fail_count = 0;
static int softreset_count = 0;

static ssize_t ahci_show_softreset_count(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "softreset_count=%d;softreset_fail_count=%d\n", softreset_count, softreset_fail_count);
}
#endif
		      ...
		      int (*check_ready)(struct ata_link *link))
{
	...
	snprintf(buf, HDD_METRICS_STR_LEN, "%s:%s:ahci_softreset_count=%d;CT;1:NR",
		 libahci_hdd_metrics_prefix, product_id2? product_id2: "def", ++softreset_count);
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	/* Log in metrics */
	snprintf(buf, HDD_METRICS_STR_LEN, "%s:%s:ahci_softreset_fail_count=%d;CT;1:NR", libahci_hdd_metrics_prefix, product_id2? product_id2: "def", ++softreset_fail_count);
	log_to_metrics(ANDROID_LOG_INFO, "HDDEvent", buf);
#endif
	return rc;
}
