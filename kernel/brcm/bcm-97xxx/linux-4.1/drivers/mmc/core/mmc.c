...
char *emmc_metrics_prefix = "frankemmc";
	...
	if (card->ext_csd.rev >= 7) {
		...
		if(buf){
			snprintf(buf, METRICS_LIFETIME_DATA_LEN,
				"%s:%s:est_life_time_type_a_%x=1,est_life_time_type_b_%x=1;CT;1:NR",
				emmc_metrics_prefix, product_id2? product_id2: "def",
				card->ext_csd.device_life_time_est_typ_a, card->ext_csd.device_life_time_est_typ_b);
			printk("log to metrics called but nothing printed");
