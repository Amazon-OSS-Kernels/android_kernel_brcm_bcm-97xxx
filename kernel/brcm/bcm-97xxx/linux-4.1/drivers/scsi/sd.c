...
static void sd_shutdown(struct device *dev)
{
	...
	/*
	 * For Frank device, HDD power is turned off and on during system restart
	 * stopping disk is required power-off sequence
	 *
	 * TODO: make sure internal HDD gets sda as disk_name
	 * typically this is the case, but we don't have rule to guarantee now
	 */
	else if (sdkp->disk && strcmp(sdkp->disk->disk_name, "sda") == 0) {
		sd_printk(KERN_NOTICE, sdkp, "Stopping disk\n");
		sd_start_stop_device(sdkp, 0);
	}
}
